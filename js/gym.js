...
function renderGym() {
  ...
  const totalVolume = dayExercises.reduce((s, ex) => s + ex.sets.reduce((v, set) => v + Number(set.reps) * Number(set.weight), 0), 0);
      ...
      <div class="gym-entry">
        <div class="gym-entry-head">
          ...
          <div class="gym-entry-right">
            <span class="gym-entry-vol">${vol.toLocaleString()} lbs</span>
        ...
        <table class="gym-sets-table">
          ...
          <tbody>
            ${ex.sets.map((s, si) => `<tr><td>${si + 1}</td><td>${s.reps}</td><td>${s.weight} lbs</td></tr>`).join('')}
          </tbody>
        </table>
      </div>`;
    }).join('');
...
function bindGymEvents() {
  ...
  $('#gymSaveExerciseBtn').addEventListener('click', () => {
    ...
    if (!name) return;
    const validSets = gymSets.filter(s => s.reps && s.weight).map(s => ({ reps: Number(s.reps), weight: Number(s.weight) }));
    if (!validSets.length) return;
    state.gym.push({ date: gymViewDate, exercise: name, sets: validSets });
    saveData(state);
